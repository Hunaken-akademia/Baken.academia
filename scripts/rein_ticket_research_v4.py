...
def generate_race(group: pd.DataFrame, mix: tuple[float, float, float]) -> pd.DataFrame:
    outputs = []
    for bet_type, frame in candidates(group, mix).items():
        remaining = frame.sort_values("probability", ascending=False).copy()
        parts = []
        for ticket_type, count in zip(GROUPS, BET_CAPS[bet_type].__dict__.values()):
...
def generate_all(runners: pd.DataFrame, mix: tuple[float, float, float]) -> pd.DataFrame:
    frames = []
    ...
    for index, (_, group) in enumerate(runners.groupby("race_id", observed=True, sort=False), 1):
        ...
        if len(clean) >= 4:
            frames.append(generate_race(clean, mix))
        if index % 500 == 0:
