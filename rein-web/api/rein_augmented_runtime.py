...
class ReinRuntime(BaseRuntime):
    def score(self, race, runners):
        ...
        reused = base.pop("_market_role_scores", None)
        if self.market_models is None or not market_inputs_valid(runners):
            base["market_difference_ready"] = False
            return base
        if reused is not None:
            # Same _market_feature_frame inputs (categoricals are converted to object
            # inside it), same models, same clip-and-normalize as score_market_difference.
            scores = reused
        else:
