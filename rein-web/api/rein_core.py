...
class ReinRuntime:
    ...
    def _market_feature_frames(
        self, full_frame: pd.DataFrame, race: dict[str, Any],
        runners: list[dict[str, Any]], targets: tuple[int, ...] = (1, 2, 3),
    ) -> dict[int, pd.DataFrame]:
        """All-target equivalent of _market_feature_frame, built in one pass.

        Only the fit_*/gate_course_bias/pace_course_style rates depend on the target;
        every other column is identical across targets and across the market/rein
        models. Arithmetic and subset definitions are copied from
        _market_feature_frame so each returned frame is exactly equal to it
        (tests/test_market_frames.py checks this).
        """
        frame = full_frame.copy()
        for column in CATEGORICAL:
            if column in frame:
        ...
        history = self.history.loc[self.history["race_date"].lt(race_date)].copy()
        finish = pd.to_numeric(history["finish_position"], errors="coerce")
        target_columns = {target: f"_target_{target}" for target in targets}
        for target, column in target_columns.items():
            history[column] = finish.eq(target).astype(float)
        history["_wet_key"] = history["going"].isin(["重", "不良"])
        historical_prior_early = history.groupby("horse_id", sort=False, observed=True)["early_pct"].shift(1)
        history["_style_key"] = np.select(
            [historical_prior_early.le(.25), historical_prior_early.le(.5), historical_prior_early.gt(.5)],
        ...
        current_wet = (race.get("going") or "") in ("重", "不良")
        distance_bucket = int(float(race["distance_m"]) // 200)
        rates: dict[int, dict[str, list[float]]] = {target: {} for target in targets}
        ...
        for index, runner in enumerate(runners):
            ...
            for subset, prefix in specs:
                n = float(len(subset))
                starts.setdefault(prefix + "_starts", []).append(n)
                for target, column in target_columns.items():
                    rates[target].setdefault(prefix + "_rate", []).append(
                        float((subset[column].sum() + 6.4) / (n + 80))
                    )
        for column, values in starts.items():
            frame[column] = pd.Series(values, index=frame.index, dtype=float)
        frame["form_speed_trend"] = frame["form_speed_last3"] - frame["form_speed_last10"]
        frame["form_surprise_trend"] = frame["form_surprise_last3"] - frame["form_surprise_last10"]
        frame["body_change_abs"] = frame["horse_weight_change"].abs()
        frame["body_change_pct"] = frame["horse_weight_change"] / frame["horse_weight"]
        ...
        for target in targets:
            ...
            for column, values in rates[target].items():
                target_frame[column] = pd.Series(values, index=frame.index, dtype=float)
            output[target] = target_frame
        return output

    def _market_frame_columns(self, frames: dict[int, pd.DataFrame], target: int, columns: list[str]) -> pd.DataFrame:
        frame = frames[target]
        missing = [column for column in columns if column not in frame]
        if missing:
            raise RuntimeError("Market model features are unavailable: " + ",".join(missing[:5]))
    ...
    def score(self, race: dict[str, Any], runners: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if self.market_models and market_inputs_valid(runners):
            market_values = {}
            rein_values = {}
            # One shared pass instead of six identical history scans (see
            # _market_feature_frames); the model inputs are unchanged.
            market_frames = self._market_feature_frames(full_frame, race, runners)
            for role, target in (("first", 1), ("second", 2), ("third", 3)):
                ...
                market_frame = self._market_frame_columns(market_frames, target, market_model.feature_name())
                rein_frame = self._market_frame_columns(market_frames, target, rein_model.feature_name())
